...
function getSessionId(): string {
  ...
  let sid = localStorage.getItem('fitgenius_session')
  if (!sid) {
    sid = 'session_' + Math.random().toString(36).substr(2, 9)
    localStorage.setItem('fitgenius_session', sid)
  }
  return sid
}
...
export default function Home() {
  ...
  async function sendMessage(text?: string) {
    ...
    if (!userMessage || loading || quota === 0) return
    ...
    try {
      const res = await axios.post(
        ...
        { message: userMessage, session_id: getSessionId(), history }
      )
  ...
  const quotaPct = (quota / 30) * 100
  const quotaHue = Math.round((quotaPct / 100) * 120) // 0=red, 120=green
      ...
      <div className="app">
        ...
        <header className="header">
          ...
          <div className="quota-area">
            <div className="quota-label">{quota}/30 questions</div>
            <div className="quota-bar-wrap">
              <div
                className="quota-bar-fill"
                style={{
                  width: `${quotaPct}%`,
                  background: `hsl(${quotaHue}, 80%, 55%)`
                }}
              />
            </div>
          </div>
          ...
          {messages.length === 0 && !loading && (
            <div className="hero">
              ...
              <div className="hero-sub">
                ...
                Ask about training, nutrition, or injury recovery.
              </div>
        ...
        {quota === 0 && (
          <div className="quota-banner">
            ⚠️ You've used all 30 questions for this period.
